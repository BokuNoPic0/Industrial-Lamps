...
public class MiniLampBlock extends BlockContainer{// BlockContainer block
    ...
    public MiniLampBlock(String name, CreativeTab tab, float lightLevel) {
    ...
    public TileEntity createNewTileEntity(World worldIn, int meta) {
        return new MiniLampICTileEntity(getStateFromMeta(meta).getValue(FACING));
    }
    @Override
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if(!worldIn.isRemote){
            TileEntity te = worldIn.getTileEntity(pos);
            if (te != null && te instanceof MiniLampICTileEntity && playerIn.capabilities.isCreativeMode) {
                playerIn.sendMessage(new TextComponentString("energy: "+((MiniLampICTileEntity) te).getEnergy()+"/"+((MiniLampICTileEntity) te).getMaxEnergy()));
            }
        }
        return true;
    }
