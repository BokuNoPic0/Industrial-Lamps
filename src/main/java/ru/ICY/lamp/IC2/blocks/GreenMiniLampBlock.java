...
import ru.ICY.lamp.IC2.TileEntities.GreenMiniLampICTileEntity;
...
public class GreenMiniLampBlock extends BlockContainer{// BlockContainer block
    ...
    public GreenMiniLampBlock(String name, CreativeTab tab, float lightLevel) {
    ...
    public TileEntity createNewTileEntity(World worldIn, int meta) {
        return new GreenMiniLampICTileEntity(getStateFromMeta(meta).getValue(FACING));
    }
    @Override
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if(!worldIn.isRemote){
            TileEntity te = worldIn.getTileEntity(pos);
            if (te != null && te instanceof GreenMiniLampICTileEntity && playerIn.capabilities.isCreativeMode) {
                playerIn.sendMessage(new TextComponentString("energy: "+((GreenMiniLampICTileEntity) te).getEnergy()+"/"+((GreenMiniLampICTileEntity) te).getMaxEnergy()));
            }
        }
        return true;
    }
