...
import ru.ICY.lamp.IC2.TileEntities.YellowLampICTileEntity;
...
public class YellowLampBlock extends BlockContainer {
    protected static final AxisAlignedBB FULL_AABB = new AxisAlignedBB(0D, 0D, 0D, 1D, 1D, 1D);
    public YellowLampBlock(String name, Material material, CreativeTab tab, float lightLevel) {
        this(name,material,lightLevel);
        this.setCreativeTab(tab);
    }
    public YellowLampBlock(String name, Material material, float lightLevel) {
        super(material);
    ...
    public TileEntity createNewTileEntity(World worldIn, int meta) {
        return new YellowLampICTileEntity();
    }
    @Override
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if(!worldIn.isRemote){
            ...
            if (te != null && te instanceof YellowLampICTileEntity && playerIn.capabilities.isCreativeMode) {
                playerIn.sendMessage(new TextComponentString("energy: "+((YellowLampICTileEntity) te).getEnergy()+"/"+((YellowLampICTileEntity) te).getMaxEnergy()));
            }
        }
        return true;
    }
