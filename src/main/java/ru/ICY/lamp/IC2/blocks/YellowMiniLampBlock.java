...
import ru.ICY.lamp.IC2.TileEntities.YellowMiniLampICTileEntity;
...
public class YellowMiniLampBlock extends BlockContainer{// BlockContainer block
    ...
    public YellowMiniLampBlock(String name, CreativeTab tab, float lightLevel) {
    ...
    public TileEntity createNewTileEntity(World worldIn, int meta) {
        return new YellowMiniLampICTileEntity(getStateFromMeta(meta).getValue(FACING));
    }
    @Override
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if(!worldIn.isRemote){
            TileEntity te = worldIn.getTileEntity(pos);
            if (te != null && te instanceof YellowMiniLampICTileEntity && playerIn.capabilities.isCreativeMode) {
                playerIn.sendMessage(new TextComponentString("energy: "+((YellowMiniLampICTileEntity) te).getEnergy()+"/"+((YellowMiniLampICTileEntity) te).getMaxEnergy()));
            }
        }
        return true;
    }
