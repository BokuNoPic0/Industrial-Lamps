...
import ru.ICY.lamp.IC2.TileEntities.RedLampICTileEntity;
...
public class RedLampBlock extends BlockContainer {
    protected static final AxisAlignedBB FULL_AABB = new AxisAlignedBB(0D, 0D, 0D, 1D, 1D, 1D);
    public RedLampBlock(String name, Material material, CreativeTab tab, float lightLevel) {
        this(name,material,lightLevel);
        this.setCreativeTab(tab);
    }
    public RedLampBlock(String name, Material material, float lightLevel) {
        super(material);
    ...
    public TileEntity createNewTileEntity(World worldIn, int meta) {
        return new RedLampICTileEntity();
    }
    @Override
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if(!worldIn.isRemote){
            ...
            if (te != null && te instanceof RedLampICTileEntity && playerIn.capabilities.isCreativeMode) {
                playerIn.sendMessage(new TextComponentString("energy: "+((RedLampICTileEntity) te).getEnergy()+"/"+((RedLampICTileEntity) te).getMaxEnergy()));
            }
        }
        return true;
    }
