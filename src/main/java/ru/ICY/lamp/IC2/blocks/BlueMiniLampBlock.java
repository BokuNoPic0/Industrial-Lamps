...
import ru.ICY.lamp.IC2.TileEntities.BlueMiniLampICTileEntity;
...
public class BlueMiniLampBlock extends BlockContainer{// BlockContainer block
    ...
    public BlueMiniLampBlock(String name, CreativeTab tab, float lightLevel) {
    ...
    public TileEntity createNewTileEntity(World worldIn, int meta) {
        return new BlueMiniLampICTileEntity(getStateFromMeta(meta).getValue(FACING));
    }
    @Override
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if(!worldIn.isRemote){
            TileEntity te = worldIn.getTileEntity(pos);
            if (te != null && te instanceof BlueMiniLampICTileEntity && playerIn.capabilities.isCreativeMode) {
                playerIn.sendMessage(new TextComponentString("energy: "+((BlueMiniLampICTileEntity) te).getEnergy()+"/"+((BlueMiniLampICTileEntity) te).getMaxEnergy()));
            }
        }
        return true;
    }
